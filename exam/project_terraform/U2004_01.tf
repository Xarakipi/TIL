...
resource "vcd_vapp_vm" "vm1" {
  ...
  name          = var.vm_name_vm1
  computer_name = var.vm_name_vm1
...
resource "vcd_nsxv_firewall_rule" "vm1_SSH" {
...
resource "vcd_nsxv_firewall_rule" "vm1_http" {
  edge_gateway   = var.vcd_org_edge_name
...
resource "vcd_nsxv_dnat" "vm1_SSH" {
  edge_gateway = var.vcd_org_edge_name
  ...
  original_address   = var.vm_pip_vm1
  original_port      = 22
  translated_address = var.vm_lip_vm1
  translated_port    = 22
  protocol           = "tcp"
...
resource "vcd_nsxv_dnat" "vm1_http" {
  edge_gateway = var.vcd_org_edge_name
  network_name = var.vcd_edge_network_name
  network_type = "ext"
